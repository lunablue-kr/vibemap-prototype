...
export const CONFIG = {
  ...
  // 태그 생애주기 (§6·§9-3 v0.5.4): 작성 → 활성(리액션 가능, 크기는 최근 창 기준) → 잠금·아카이브
  // 크기는 "최근 N일" 롤링이라 주간 리셋 절벽이 없고, 오래된 태그는 자연 침전 후 기록으로 남는다.
  ...
  // 지역 오픈 임계값 (§5): 이 인원이 오픈 투표하면 그 도시 지도를 연다. 오픈 조건 공개로 참여·공유 유도.
  // 100 — 니치 미니앱의 한 지역 유입은 소수라 1,000은 비현실(영영 미오픈=기대감 좌절). 낮게 시작해
  // "지역 열림" 초기 성공을 만들고, 출시 후 투표 속도 보고 상향. config값
  REGION_OPEN_THRESHOLD: 100,
